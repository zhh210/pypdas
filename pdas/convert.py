...
'''
    Utility to convert between cvxopt and numpy/scipy.sparse matrices.
    Copied from http://maggotroot.blogspot.ch/2013/11/constrained-linear-least-squares-in.html
...
def spmatrix_to_scipy(A):
    ...
    rows = np.array(A.I).squeeze()
    cols = np.array(A.J).squeeze()
    return sparse.csc_matrix( (data, (rows, cols)),shape=A.size)
